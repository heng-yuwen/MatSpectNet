...
class MST_Plus_PlusV2(nn.Module):
    def __init__(self, in_channels=3, out_channels=31, n_feat=31, stage=3, extrat_features=False):
        super(MST_Plus_PlusV2, self).__init__()
        self.extrat_features = extrat_features
        self.stage = stage
    ...
    def forward(self, x):
        ...
        for layer in self.body:
            x, fea_mid = layer(x)
            fea_mids.append(fea_mid)
        body = x  # 31 channel
        h = self.conv_out(body)
        h += x
        if not self.extrat_features:
            return h[:, :, :h_inp, :w_inp]
        else:
            return h[:, :, :h_inp, :w_inp], fea_mids[1] # use the middle one.
